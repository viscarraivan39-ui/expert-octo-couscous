// Pipeline de video AvíspateYa (vertical 9:16) — mismo motor que
// geeknoticias/scripts/video-pipeline, adaptado a esta marca:
// guion -> TTS (Edge TTS, voz Catalina) -> imágenes (FLUX/fotos reales) ->
// ffmpeg (zoompan, subtítulos, crossfade, viñeta/grano, marca de agua) -> video.
...
const LOGO_PATH = "C:\\Users\\Maxi\\Nueva carpeta\\expert-octo-couscous\\img\\logo.png";
const DOMINIO = "avispateya.cl";
...
async function aplicarOverlaysDeMarca(inputName, outName) {
  const filter = [
    ...
    `[v1]drawtext=fontfile='${FONT}':text='${DOMINIO}':fontcolor=white@0.55:fontsize=32:borderw=2:bordercolor=black@0.4:x=w-text_w-30:y=h-70:enable='gte(t,1)'[vout]`,
  ].join(";");
