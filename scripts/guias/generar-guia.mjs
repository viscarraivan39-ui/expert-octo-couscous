// Genera guías PDF de práctica PAES M1 con identidad AvíspateYa / Profe Emi:
// logo, código QR (folio de la guía) y numeración de preguntas.
...
async function generarGuia(guia, logoBytes) {
  ...
  function drawHeader() {
    ...
    page.drawText(`Guía ${guia.numero} · ${guia.tema}`, { x: MARGIN, y, size: 17, font: fontBold, color: INK });
    y -= 16;
  ...
  function drawHojaRespuestas() {
    ...
    page.drawText(`Guía ${guia.numero} · ${guia.tema}`, { x: MARGIN + logoDims.width + 8, y: y - 25, size: 9, font: fontRegular, color: DIM });
    y -= 42;
    page.drawLine({ start: { x: MARGIN, y }, end: { x: PAGE_W - MARGIN, y }, thickness: 1.2, color: BRAND });
    y -= 26;
  ...
  pages.forEach((p, i) => {
    p.drawText(
      `AvispateYa.cl — Profe Emi · Contenido de práctica en base a preguntas oficiales estilo PAES · declarado como apoyo generado con IA · pág. ${i + 1}/${pages.length}`,
...
async function main() {
  ...
  for (const guia of GUIAS) {
    const bytes = await generarGuia(guia, logoBytes);
